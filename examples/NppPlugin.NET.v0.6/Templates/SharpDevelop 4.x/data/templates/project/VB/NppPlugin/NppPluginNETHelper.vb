...
Namespace NppPluginNET
	...
	Public Structure NppData
		Public _nppHandle As IntPtr
		Public _scintillaMainHandle As IntPtr
		Public _scintillaSecondHandle As IntPtr
	End Structure

	Public Delegate Sub NppFuncItemDelegate()

	<StructLayout(LayoutKind.Sequential)> _
	Public Structure ShortcutKey
		Public Sub New(isCtrl As Boolean, isAlt As Boolean, isShift As Boolean, key As Keys)
		...
		End Sub
		Public _isCtrl As Byte
		Public _isAlt As Byte
		Public _isShift As Byte
		Public _key As Byte
	End Structure

	<StructLayout(LayoutKind.Sequential, CharSet := CharSet.Unicode)> _
	Public Structure FuncItem
		<MarshalAs(UnmanagedType.ByValTStr, SizeConst := 64)> _
		Public _itemName As String
		Public _pFunc As NppFuncItemDelegate
		Public _cmdID As Integer
		Public _init2Check As Boolean
		Public _pShKey As ShortcutKey
	End Structure

	Public Class FuncItems
		Implements IDisposable
		Private _funcItems As List(Of FuncItem)
		Private _sizeFuncItem As Integer
		Private _shortCutKeys As List(Of IntPtr)
		Private _nativePointer As IntPtr
		Private _disposed As Boolean = False

		Public Sub New()
			_funcItems = New List(Of FuncItem)()
			_sizeFuncItem = Marshal.SizeOf(GetType(FuncItem))
			_shortCutKeys = New List(Of IntPtr)()
		End Sub

		<DllImport("kernel32")> _
		Private Shared Sub RtlMoveMemory(Destination As IntPtr, Source As IntPtr, Length As Integer)
		End Sub
		Public Sub Add(funcItem As FuncItem)
			Dim oldSize As Integer = _funcItems.Count * _sizeFuncItem
			_funcItems.Add(funcItem)
			Dim newSize As Integer = _funcItems.Count * _sizeFuncItem
			Dim newPointer As IntPtr = Marshal.AllocHGlobal(newSize)

			If _nativePointer <> IntPtr.Zero Then
				RtlMoveMemory(newPointer, _nativePointer, oldSize)
				Marshal.FreeHGlobal(_nativePointer)
			End If
			Dim ptrPosNewItem As IntPtr = CType(CInt(newPointer) + oldSize, IntPtr)
			Dim aB As Byte() = Encoding.Unicode.GetBytes(funcItem._itemName & vbNullChar)
			Marshal.Copy(aB, 0, ptrPosNewItem, aB.Length)
			ptrPosNewItem = CType(CInt(ptrPosNewItem) + 128, IntPtr)
			Dim p As IntPtr =  IntPtr.Zero
			If (funcItem._pFunc IsNot Nothing) Then
				p = Marshal.GetFunctionPointerForDelegate(funcItem._pFunc)
			End If
			Marshal.WriteIntPtr(ptrPosNewItem, p)
			ptrPosNewItem = CType(CInt(ptrPosNewItem) + IntPtr.Size, IntPtr)
			Marshal.WriteInt32(ptrPosNewItem, funcItem._cmdID)
			ptrPosNewItem = CType(CInt(ptrPosNewItem) + 4, IntPtr)
			Marshal.WriteInt32(ptrPosNewItem, Convert.ToInt32(funcItem._init2Check))
			ptrPosNewItem = CType(CInt(ptrPosNewItem) + 4, IntPtr)
			If funcItem._pShKey._key <> 0 Then
				Dim newShortCutKey As IntPtr = Marshal.AllocHGlobal(4)
		...
		Public Sub RefreshItems()
			Dim ptrPosItem As IntPtr = _nativePointer
			For i As Integer = 0 To _funcItems.Count - 1
				Dim updatedItem As New FuncItem()
				updatedItem._itemName = _funcItems(i)._itemName
				ptrPosItem = CType(CInt(ptrPosItem) + 128, IntPtr)
				updatedItem._pFunc = _funcItems(i)._pFunc
				ptrPosItem = CType(CInt(ptrPosItem) + IntPtr.Size, IntPtr)
				updatedItem._cmdID = Marshal.ReadInt32(ptrPosItem)
				ptrPosItem = CType(CInt(ptrPosItem) + 4, IntPtr)
		...
		Public ReadOnly Property NativePointer() As IntPtr
			...
			End Get
		End Property
		Public ReadOnly Property Items() As List(Of FuncItem)
			Get
				Return _funcItems
			End Get
		End Property
		...
		Public Sub Dispose() Implements IDisposable.Dispose
			If Not _disposed Then
				For Each ptr As IntPtr In _shortCutKeys
	...
	<Flags> _
	Public Enum NppTbMsg As UInteger
		...
		DWS_DF_FLOATING = &H80000000UI
		' default state is floating
	End Enum

	<StructLayout(LayoutKind.Sequential, CharSet := CharSet.Unicode)> _
	Public Structure NppTbData
		Public hClient As IntPtr
		' HWND: client Window Handle
		Public pszName As String
		' TCHAR*: name of plugin (shown in window)
		Public dlgID As Integer
		' int: a funcItem provides the function pointer to start a dialog. Please parse here these ID
		' user modifications
		Public uMask As NppTbMsg
		' UINT: mask params: look to above defines
		Public hIconTab As UInteger
		' HICON: icon for tabs
		Public pszAddInfo As String
		' TCHAR*: for plugin to display additional informations
		' internal data, do not use !!!
		Public rcFloat As RECT
		' RECT: floating position
		Public iPrevCont As Integer
		' int: stores the privious container (toggling between float and dock)
		Public pszModuleName As String
		' const TCHAR*: it's the plugin file name. It's used to identify the plugin
	End Structure
	...
	<Flags> _
	Public Enum NppMsg As UInteger
...
'        
	...
	Public Enum NppMenuCmd As UInteger
		IDM = 40000
	...
	<Flags> _
	Public Enum DockMgrMsg As UInteger
		IDB_CLOSE_DOWN = 137
		IDB_CLOSE_UP = 138
	...
	<StructLayout(LayoutKind.Sequential)> _
	Public Structure toolbarIcons
		Public hToolbarBmp As IntPtr
		Public hToolbarIcon As IntPtr
	End Structure
	#End Region

	#Region " Scintilla "
	<StructLayout(LayoutKind.Sequential)> _
	Public Structure Sci_NotifyHeader
...
'         * but most clients of Scintilla.h do not have this type visible. 
	...
	Public Structure SCNotification
		Public nmhdr As Sci_NotifyHeader
		...
		' SCN_MODIFIED 
		Public foldLevelNow As Integer
		' SCN_MODIFIED 
		Public foldLevelPrev As Integer
		' SCN_MODIFIED 
		...
		Public annotationLinesAdded As Integer
		' SC_MOD_CHANGEANNOTATION 
	End Structure

	<Flags> _
	Public Enum SciMsg As UInteger
		INVALID_POSITION = &HffffffffUI
		SCI_START = 2000
		...
		SC_FOLDFLAG_LINEBEFORE_EXPANDED = &H2
		SC_FOLDFLAG_LINEBEFORE_CONTRACTED = &H4
		SC_FOLDFLAG_LINEAFTER_EXPANDED = &H8
		SC_FOLDFLAG_LINEAFTER_CONTRACTED = &H10
		SC_FOLDFLAG_LEVELNUMBERS = &H40
		...
		SC_WRAPVISUALFLAG_NONE = &H0
		SC_WRAPVISUALFLAG_END = &H1
		SC_WRAPVISUALFLAG_START = &H2
		SCI_SETWRAPVISUALFLAGS = 2460
		SCI_GETWRAPVISUALFLAGS = 2461
		SC_WRAPVISUALFLAGLOC_DEFAULT = &H0
		SC_WRAPVISUALFLAGLOC_END_BY_TEXT = &H1
		SC_WRAPVISUALFLAGLOC_START_BY_TEXT = &H2
		...
		SC_MULTISTEPUNDOREDO = &H80
		SC_LASTSTEPINUNDOREDO = &H100
		SC_MOD_CHANGEMARKER = &H200
		SC_MOD_BEFOREINSERT = &H400
		SC_MOD_BEFOREDELETE = &H800
		SC_MULTILINEUNDOREDO = &H1000
		SC_STARTACTION = &H2000
		SC_MOD_CHANGEINDICATOR = &H4000
		SC_MOD_CHANGELINESTATE = &H8000
		SC_MOD_CHANGEMARGIN = &H10000
		SC_MOD_CHANGEANNOTATION = &H20000
		SC_MOD_CONTAINER = &H40000
		SC_MODEVENTMASKALL = &H7ffff
	...
	<StructLayout(LayoutKind.Sequential)> _
	Public Structure Sci_CharacterRange
		Public Sub New(cpmin__1 As Integer, cpmax__2 As Integer)
			cpMin = cpmin__1
			cpMax = cpmax__2
		End Sub
	...
	Public Class Sci_TextRange
		Implements IDisposable
		Private _sciTextRange As _Sci_TextRange
		Private _ptrSciTextRange As IntPtr
		Private _disposed As Boolean = False

		Public Sub New(chrRange As Sci_CharacterRange, stringCapacity As Integer)
			_sciTextRange.chrg = chrRange
			_sciTextRange.lpstrText = Marshal.AllocHGlobal(stringCapacity)
		End Sub
		Public Sub New(cpmin As Integer, cpmax As Integer, stringCapacity As Integer)
		...
		<StructLayout(LayoutKind.Sequential)> _
		Private Structure _Sci_TextRange
			Public chrg As Sci_CharacterRange
		...
		Public ReadOnly Property NativePointer() As IntPtr
			...
			End Get
		End Property
		Public ReadOnly Property lpstrText() As String
			Get
				_readNativeStruct()
				Return Marshal.PtrToStringAnsi(_sciTextRange.lpstrText)
			End Get
		End Property
		Public Property chrg() As Sci_CharacterRange
			Get
				_readNativeStruct()
			...
			End Set
		End Property
		Private Sub _initNativeStruct()
			If _ptrSciTextRange = IntPtr.Zero Then
		...
		End Sub
		Private Sub _readNativeStruct()
			If _ptrSciTextRange <> IntPtr.Zero Then
				_sciTextRange = CType(Marshal.PtrToStructure(_ptrSciTextRange, GetType(_Sci_TextRange)), _Sci_TextRange)
			End If
		End Sub

		Public Sub Dispose() Implements IDisposable.Dispose
			If Not _disposed Then
				If _sciTextRange.lpstrText <> IntPtr.Zero Then
	...
	Public Class Sci_TextToFind
		Implements IDisposable
		Private _sciTextToFind As _Sci_TextToFind
		Private _ptrSciTextToFind As IntPtr
		Private _disposed As Boolean = False

		Public Sub New(chrRange As Sci_CharacterRange, searchText As String)
			_sciTextToFind.chrg = chrRange
			_sciTextToFind.lpstrText = Marshal.StringToHGlobalAnsi(searchText)
		End Sub
		Public Sub New(cpmin As Integer, cpmax As Integer, searchText As String)
		...
		<StructLayout(LayoutKind.Sequential)> _
		Private Structure _Sci_TextToFind
			Public chrg As Sci_CharacterRange
			Public lpstrText As IntPtr
			Public chrgText As Sci_CharacterRange
		End Structure
		...
		Public ReadOnly Property NativePointer() As IntPtr
			...
			End Get
		End Property
		Public WriteOnly Property lpstrText() As String
			Set
				_freeNativeString()
		...
		Public Property chrg() As Sci_CharacterRange
			...
			End Set
		End Property
		Public ReadOnly Property chrgText() As Sci_CharacterRange
			Get
				_readNativeStruct()
		...
		End Property
		Private Sub _initNativeStruct()
			If _ptrSciTextToFind = IntPtr.Zero Then
		...
		Private Sub _readNativeStruct()
			If _ptrSciTextToFind <> IntPtr.Zero Then
				_sciTextToFind = CType(Marshal.PtrToStructure(_ptrSciTextToFind, GetType(_Sci_TextToFind)), _Sci_TextToFind)
			End If
		End Sub
		Private Sub _freeNativeString()
			If _sciTextToFind.lpstrText <> IntPtr.Zero Then
	...
	Public Class Win32
		<DllImport("user32")> _
		Public Shared Function SendMessage(hWnd As IntPtr, Msg As NppMsg, wParam As Integer, lParam As NppMenuCmd) As IntPtr
		End Function
		<DllImport("user32")> _
		Public Shared Function SendMessage(hWnd As IntPtr, Msg As NppMsg, wParam As Integer, lParam As IntPtr) As IntPtr
		End Function
		<DllImport("user32")> _
		Public Shared Function SendMessage(hWnd As IntPtr, Msg As NppMsg, wParam As Integer, lParam As Integer) As IntPtr
		End Function
		'<DllImport("user32")> _
		'Public Shared Function SendMessage(hWnd As IntPtr, Msg As NppMsg, wParam As Integer, ByRef lParam As Integer) As IntPtr
		'End Function
		<DllImport("user32")> _
		Public Shared Function SendMessage(hWnd As IntPtr, Msg As NppMsg, wParam As IntPtr, lParam As Integer) As IntPtr
		End Function
		<DllImport("user32")> _
		Public Shared Function SendMessage(hWnd As IntPtr, Msg As NppMsg, wParam As Integer, ByRef lParam As LangType) As IntPtr
		End Function
		<DllImport("user32")> _
		Public Shared Function SendMessage(hWnd As IntPtr, Msg As NppMsg, wParam As Integer, <MarshalAs(UnmanagedType.LPWStr)> lParam As StringBuilder) As IntPtr
		End Function
		<DllImport("user32")> _
		Public Shared Function SendMessage(hWnd As IntPtr, Msg As NppMsg, wParam As Integer, <MarshalAs(UnmanagedType.LPWStr)> lParam As String) As IntPtr
		End Function
		<DllImport("user32")> _
		Public Shared Function SendMessage(hWnd As IntPtr, Msg As NppMsg, wParam As IntPtr, <MarshalAs(UnmanagedType.LPWStr)> lParam As String) As IntPtr
		End Function

		<DllImport("user32")> _
		Public Shared Function SendMessage(hWnd As IntPtr, Msg As SciMsg, wParam As Integer, lParam As IntPtr) As IntPtr
		End Function
		<DllImport("user32")> _
		Public Shared Function SendMessage(hWnd As IntPtr, Msg As SciMsg, wParam As Integer, lParam As String) As IntPtr
		End Function
		<DllImport("user32")> _
		Public Shared Function SendMessage(hWnd As IntPtr, Msg As SciMsg, wParam As Integer, <MarshalAs(UnmanagedType.LPStr)> lParam As StringBuilder) As IntPtr
		End Function
		<DllImport("user32")> _
		Public Shared Function SendMessage(hWnd As IntPtr, Msg As SciMsg, wParam As Integer, lParam As Integer) As IntPtr
		End Function

		Public Const MAX_PATH As Integer = 260
		<DllImport("kernel32")> _
		Public Shared Function GetPrivateProfileInt(lpAppName As String, lpKeyName As String, nDefault As Integer, lpFileName As String) As Integer
		End Function
		<DllImport("kernel32")> _
		...
		Public Const MF_CHECKED As Integer = 8
		Public Const MF_UNCHECKED As Integer = 0
		<DllImport("user32")> _
		Public Shared Function GetMenu(hWnd As IntPtr) As IntPtr
		End Function
		<DllImport("user32")> _
		Public Shared Function CheckMenuItem(hmenu As IntPtr, uIDCheckItem As Integer, uCheck As Integer) As Integer
		End Function

		Public Const WM_CREATE As Integer = 1
	End Class

	Public Class ClikeStringArray
		Implements IDisposable
		Private _nativeArray As IntPtr
		Private _nativeItems As List(Of IntPtr)
		Private _disposed As Boolean = False

		Public Sub New(num As Integer, stringCapacity As Integer)
			_nativeArray = Marshal.AllocHGlobal((num + 1) * IntPtr.Size)
			_nativeItems = New List(Of IntPtr)()
			For i As Integer = 0 To num - 1
				Dim item As IntPtr = Marshal.AllocHGlobal(stringCapacity)
				Marshal.WriteIntPtr(CType(CInt(_nativeArray) + (i * IntPtr.Size), IntPtr), item)
				_nativeItems.Add(item)
			Next
			Marshal.WriteIntPtr(CType(CInt(_nativeArray) + (num * IntPtr.Size), IntPtr), IntPtr.Zero)
		End Sub
		...
		Public ReadOnly Property NativePointer() As IntPtr
			...
			End Get
		End Property
		Public ReadOnly Property ManagedStringsAnsi() As List(Of String)
			Get
				Return _getManagedItems(False)
			End Get
		End Property
		Public ReadOnly Property ManagedStringsUnicode() As List(Of String)
			Get
				Return _getManagedItems(True)
			End Get
		End Property
		Private Function _getManagedItems(unicode As Boolean) As List(Of String)
			Dim _managedItems As New List(Of String)()
			For i As Integer = 0 To _nativeItems.Count - 1
				If unicode Then
		...
		Public Sub Dispose() Implements IDisposable.Dispose
			If Not _disposed Then
				For i As Integer = 0 To _nativeItems.Count - 1
					If _nativeItems(i) <> IntPtr.Zero Then
						Marshal.FreeHGlobal(_nativeItems(i))
					End If
				Next
				If _nativeArray <> IntPtr.Zero Then
